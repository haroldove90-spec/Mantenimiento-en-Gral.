    ...
    <div id="tech-module" className="w-full px-4 sm:px-6 py-4 space-y-5">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto justify-end">
          ...
          <button
            onClick={() => {
              if (window.confirm('⚠️ ¿Deseas eliminar las órdenes y registros de muestra del sistema?')) {
                clearSampleData();
              }
            }}
            className="bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center space-x-1.5 cursor-pointer shrink-0"
            title="Borrar órdenes y datos de muestra"
          >
      ...
      <div className="space-y-4">
        ...
        {sortedOrders.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-2xl text-center py-16 text-slate-400 space-y-3 shadow-xs">
            <CheckCircle2 className="w-12 h-12 text-slate-300 mx-auto" />
            <p className="font-semibold text-slate-700 text-base">No hay órdenes en esta categoría para el técnico seleccionado.</p>
            <button
              onClick={() => setStatusFilter('all')}
